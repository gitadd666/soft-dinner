...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // caso inicial: llega desde Restablecer o desde un forward con correo + codigo
    if (isset($_POST['correo']) && isset($_POST['codigo']) && !isset($_POST['verify']) && !isset($_POST['action'])) {
        // almacenar en sesión para reenvíos y verificación posterior (restablecimiento)
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'resend') {
        if (!empty($_SESSION['reset_correo']) && !empty($_SESSION['reset_codigo'])) {
            ...
            $res = enviarCodigoPorCorreo($_SESSION['create_correo'], $_SESSION['create_codigo']);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode($res);
            exit;
        } else {
    ...
    if (isset($_POST['verify'])) {
        ...
        if (!empty($_SESSION['create_codigo'])) {
            if ($codigoUsuario === $_SESSION['create_codigo']) {
                // insertar nuevo usuario usando los datos guardados en sesión
                $nombre = isset($_SESSION['create_nombre']) ? $_SESSION['create_nombre'] : '';
                $correo = isset($_SESSION['create_correo']) ? $_SESSION['create_correo'] : '';
                $contrasena = isset($_SESSION['create_contrasena']) ? $_SESSION['create_contrasena'] : '';
                ...
                if ($nombre !== '' && $correo !== '') {
                    ...
                    // limpiar sesión de datos de creación
            ...
            // si no hay flujo reconocido, marcar error
            $errorCodigo = true;
        }
    }
}
...
<html lang="es">
  <head>
    <meta charset="UTF-8">
    <title>VERIFICACIÓN - SOFT DINNER</title>
